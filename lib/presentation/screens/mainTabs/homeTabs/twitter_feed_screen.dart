...
class TwitterFeedScreenState extends State<TwitterFeedScreen> {
  ...
  Widget build(BuildContext context) => FutureBuilder(
      future: GetTweets(TwitterRepositoryImp())(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting)
          return SliverFillRemaining(
              child: Center(child: CircularProgressBar()));
        _tweets = snapshot.data as List<Tweet>;
...
class TweetItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) => InkWell(
        ...
        child: Card(
            ...
            child: Container(
              padding:
                  const EdgeInsets.only(left: 7, right: 7, bottom: 24, top: 12),
                  ...
                  LinkifyText(
                    ...
                    onTap: (link) async {
                      if (link.value![0] == '#')
                        ...
                        await launch(
                            'https://twitter.com/${link.value!.substring(1)}');
